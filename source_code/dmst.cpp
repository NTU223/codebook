...
int n,m,incycle[MAXNUM]={0},cycid;
bool contracted[MAXNUM];
inline int dmst(int s){
  int v,u,i,cost=0;
  for(i=0;i<n;i++) { pred[i].v=-1; contracted[i]=0; }
  while(1) {
    // find a uncontracted node with no in arc: v
    for(v=0;v<n;v++)
      if(v!=s&&!contracted[v]&&pred[v].v==-1) break;
    if(v==n) return cost; // done
    // find least-weighted in arc
    pred[v].l=INF;
    for(i=0;i<m;i++)
      if(e[i].u==v&&e[i].l<pred[v].l) pred[v]=e[i];
    if(pred[v].l==INF) return NO_SOLUTION;
