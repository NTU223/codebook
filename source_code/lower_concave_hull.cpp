...
class Segment { public:
    ...
    bool flag;
    Segment(long double _m,long double _c,long double _x1 = -INF,long double _x2=INF,bool _flag=0)
        :m(_m),c(_c),x1(_x1),x2(_x2),flag(_flag) {}
    long double evaly(long double x) const { return m*x+c; }
    const bool operator<(long double x) const { return x2-eps<x; }
    const bool operator < (const Segment &b) const {
        if(flag||b.flag) return *this<b.x1;
        return m+eps<b.m;
    }
};
class LowerConcaveHull { public: // maintain a hull like: \__/
    set<Segment> hull;
    /* functions */
    long double xintersection(Segment a,Segment b){ return (a.c-b.c)/(b.m-a.m); }
